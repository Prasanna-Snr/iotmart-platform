  ...
  return (
    <div className="border-b border-[#F0E9E3] last:border-0">
      <button
        ...
        aria-expanded={open}
        aria-controls={`faq-answer-${q.slice(0, 20).replace(/\s+/g, "-").toLowerCase()}`}
      >
        <span className="font-medium text-[#11100E] text-sm">{q}</span>
        {open
          ? <ChevronUp size={16} className="text-[#899581] flex-shrink-0" aria-hidden />
          : <ChevronDown size={16} className="text-[#899581] flex-shrink-0" aria-hidden />}
      </button>
      {open && (
        <p
          id={`faq-answer-${q.slice(0, 20).replace(/\s+/g, "-").toLowerCase()}`}
          className="pb-4 text-sm text-[#899581] leading-relaxed"
        >
          {a}
        </p>
      )}
...
export default function FAQClient() {
  return (
    <div id="main-content" className="container-custom py-12">
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        {sections.map((sec) => (
          <div key={sec.title} className="bg-white rounded-xl border border-[#CDBBAD]/50 px-6">
            ...
            {sec.items.map((item) => <FAQItem key={item.q} q={item.q} a={item.a} />)}
          </div>
        ))}
      </div>
    </div>
