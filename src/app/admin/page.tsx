...
import { salesStats, orders, revenueChartData, topProductsData } from "@/data/orders";
...
export default function AdminDashboardPage() {
  const stats = [
    ...
    },
    {
      ...
      change: salesStats.customersChange,
      icon: Users,
              ...
              <div key={d.label}>
                ...
                <div className="h-2 bg-[#F0E9E3] rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: `${(d.value / maxSales) * 100}%` }}
                  />
                </div>
              </div>
            ))}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-[#F0E9E3]">
              <tr>
                <th className="px-4 py-2.5 text-left text-xs font-semibold text-[#899581]">
                  Order
                </th>
                <th className="px-4 py-2.5 text-left text-xs font-semibold text-[#899581]">
                  Customer
                </th>
                <th className="px-4 py-2.5 text-left text-xs font-semibold text-[#899581] hidden sm:table-cell">
                  Date
                </th>
                <th className="px-4 py-2.5 text-left text-xs font-semibold text-[#899581]">
                  Total
              ...
              {orders.map((order) => (
                <tr key={order.id} className="hover:bg-[#F0E9E3]/30 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium capitalize ${ORDER_STATUS_COLORS[order.status]}`}
                    >
                      {order.status}
                    </span>
                  </td>
