...
async def create_brand(body: BrandCreate, db: AsyncSession = Depends(get_db), _=Depends(get_current_admin)):
...
async def update_brand(id: str, body: BrandUpdate, db: AsyncSession = Depends(get_db), _=Depends(get_current_admin)):
...
async def delete_brand(id: str, db: AsyncSession = Depends(get_db), _=Depends(get_current_admin)):
    result = await db.execute(select(Brand).where(Brand.id == id))
    brand = result.scalar_one_or_none()
    if not brand:
        raise HTTPException(status_code=404, detail="Brand not found")
