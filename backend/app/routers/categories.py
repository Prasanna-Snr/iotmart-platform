...
async def create_category(body: CategoryCreate, db: AsyncSession = Depends(get_db), _=Depends(get_current_admin)):
...
async def update_category(id: str, body: CategoryUpdate, db: AsyncSession = Depends(get_db), _=Depends(get_current_admin)):
...
async def delete_category(id: str, db: AsyncSession = Depends(get_db), _=Depends(get_current_admin)):
    result = await db.execute(select(Category).where(Category.id == id))
    category = result.scalar_one_or_none()
    if not category:
        raise HTTPException(status_code=404, detail="Category not found")
