...
async def create_banner(body: BannerCreate, db: AsyncSession = Depends(get_db), admin=Depends(get_current_admin)):
    banner = Banner(**body.model_dump())
    db.add(banner)
    await db.flush()
    await audit(db, admin, "banner.create", "banner", banner.id, banner.title)
    return BannerOut.model_validate(banner)


@router.put("/{id}", response_model=BannerOut)
async def update_banner(id: str, body: BannerUpdate, db: AsyncSession = Depends(get_db), admin=Depends(get_current_admin)):
    ...
    await audit(db, admin, "banner.update", "banner", banner.id, banner.title)
    return BannerOut.model_validate(banner)


@router.delete("/{id}", status_code=204)
async def delete_banner(id: str, db: AsyncSession = Depends(get_db), admin=Depends(get_current_admin)):
    result = await db.execute(select(Banner).where(Banner.id == id))
    banner = result.scalar_one_or_none()
    if not banner:
        raise HTTPException(status_code=404, detail="Banner not found")
    await audit(db, admin, "banner.delete", "banner", banner.id, banner.title)
